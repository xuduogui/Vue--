...
export var actions = {
  // 告诉后台删除某个好友聊天入口列表
  deleteChatList: function deleteChatList(context, list) {
    axios({
      method: 'post',
      url: 'chatOut.action',
  ...
  // 获得聊天栏数据，包括聊天记录
  getChatData: function getChatData(context) {
    axios({
      method: 'post',
      url: 'load.action',
  ...
  // 接受好友消息，成功回调
  receiveMsg: function receiveMsg(context) {
    axios({
      method: 'post',
      url: 'receive.action',
      data: { "userId": state.mycookie }
  ...
  // 发送消息到后台
  sendMyMsg: function sendMyMsg(context, msg) {
    axios({
      method: 'post',
      url: 'send.action',
      data: msg
  ...
  // 发起聊天向后台传递列表创建数据
  sendChatList: function sendChatList(context, msg) {
    axios({
      method: 'post',
      url: 'chatIn.action',
      data: msg
